...
namespace Nuclear.Test.Worker.Dummies {
    internal class TestClass {
        ...
        internal void Method_NoArgs() {
            TestResult.ActionResult.AppendLine($"{nameof(MethodInfo_NoArgs)}()");
        }

        internal void Method_OneArg(String arg1) {
            TestResult.ActionResult.AppendLine($"{nameof(Method_OneArg)}({arg1.Format()})");
        }

        internal void Method_TwoArgs(String arg1, Int32 arg2) {
            TestResult.ActionResult.AppendLine($"{nameof(Method_TwoArgs)}({arg1.Format()}, {arg2.Format()})");
        }

        internal void Method_OneGeneric_NoArgs<T>() {
            TestResult.ActionResult.AppendLine($"{nameof(Method_OneGeneric_NoArgs)}<{typeof(T).Format()}>()");
        }

        internal void Method_OneGeneric_OneArg<T>(String arg1) {
            TestResult.ActionResult.AppendLine($"{nameof(Method_OneGeneric_OneArg)}<{typeof(T).Format()}>({arg1.Format()})");
        }

        internal void Method_TwoGeneric_TwoArgs<T1, T2>(String arg1, Int32 arg2) {
            TestResult.ActionResult.AppendLine($"{nameof(Method_TwoGeneric_TwoArgs)}<{typeof(T1).Format()}, {typeof(T2).Format()}>({arg1.Format()}, {arg2.Format()})");
        }
